...
public class PostDao extends Dao {
    ...
    public int getTotalCountSearch( int cno , String key , String keyword ){
        try {
            ...
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1,cno);
            ps.setString(2, "%" + keyword + "%"); // sql에서 포함된 비교는 like %키워드% 사용된다.
            ResultSet rs = ps.executeQuery();
    ...
    public PostDto getPost(int pno){
        try {
            ...
            if (rs.next()){
                ...
                dto.setMno( rs.getInt("mno") );             dto.setCno( rs.getInt( "cno" ));
                dto.setPcontent( rs.getString("pcontent")); dto.setPdate( rs.getString("pdate"));
                dto.setPview( rs.getInt("pview"));          dto.setPno( rs.getInt("pno"));
                dto.setPtitle( rs.getString("ptitle"));     dto.setMid( rs.getString("mid") );
                return dto;
            }
