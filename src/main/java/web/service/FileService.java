...
public class FileService {
    ...
    // 방법2) 프로젝트 내 build : 서버를 실행했을 때 컴파일 된 코드 즉] 내장서버가 갖는 실행된 폴더
    ...
    public String fileUpload(MultipartFile multipartFile ){
        ...
        // 1. 업로드한 파일명이 중복일 때, 다른 파일이지만 파일명은 같을 수 있다.
            ...
            // 방법2) 파일명 앞에 업로드된 날짜/시간 붙인다.
                // ----> 파일 받을 때 파일명이 달라짐
            // 방법3) UUID(네트워크식별번호) 라이브러리 사용한다. UUID : 난수 문자열 생성 (고유성 보장)
        // (1) 방법3처럼 UUID를 생성한다.
        String uuid = UUID.randomUUID().toString();
        // (2) 업로드된 파일명과 합치기    , .getOriginalFilename(); : 업로드된 파일명
            // - 업로드된 파일명에 _언더바 존재하면 모두 -하이픈으로 변경 . 예) 짱구_사진.jpg ===> 짱구-사진.jpg
            // 하는 이유 : uuid 와 파일명 구분을 _언더바 사용하기 위한 내부적인 규칙
            // 예상 :
        String fileName = uuid + "_" + multipartFile.getOriginalFilename().replaceAll("_" , "-");
        // .replaceAll("기존문자" , "새로운문자"): 기존문자를 새로운문자로 변환해주는 메소드
        ...
        // 5. 업로드( 파일/바이트 이동 ) 하기 , .transferTo( file객체 ) : 지정한 file 객체의 경로로 업로드 파일을 이동한다.
        try {
            multipartFile.transferTo(uploadFile); // multipartFile(업로드된 파일)
            // transferTo : 꼭 예외처리 해줘야함
        }catch (Exception e ){
    ...
    public void fileDownload(String fileName , HttpServletResponse response ){
        ...
        // 매개변수2 : HttpServletResponse response 다운로드를 요청한 사용자의 *응답* 객체 , 요청과 응답의 사용자는 동일
        ...
        try { // 예외처리 필수
            ...
            fin.close(); // 필수는 아니지만 대용량에서는 안전하게 입력스트림 객체를 직접 닫기 권장!

        // ******************* 다운로드 형식 지정 : 브라우저 마다 상이 ******************** //
        // - 파일의 uuid 제거 , .split("기존문자") : 문자열내 기준문자 기준으로 문자열 자르기
        String oldFilename = fileName.split("_")[1]; // UUID-파일명 _언더바 기준으로 잘라서 2번째 문자열 뜻
        // - 응답 형식 지정하기 , 다운로드 형식과 다운로드 파일지정
        ...
        // 4. 업로드할 파일을 사용자에게 (response 객체 이용한) 응답하기.
            ...
            // 4-2. '3-3'에서 읽어온 바이트 배열을 다운로드를 요청한 (response) 사용자에게 출력(쓰기) 하기
            fout.write(bytes);
            // 4-3. 안전하게 스트림 닫기
            fout.close(); // 필수는 아니지만 대용량에서는 안전하게 출력스트림 객체를 직접 닫기 권장!
